...
def lddt_loss(out, batch, config):
    ...
    ####### Calculate loss separately for loop######
    loss_renum_loop = torch.sum(errors * mask_renum, dim=-1) / (
            1e-10 + torch.sum(mask_renum, dim=-1)
    )
    loss_renum_loop = loss_renum_loop * (
            (batch['resolution'] >= config['predicted_lddt']['min_resolution']) & (
            batch['resolution'] <= config['predicted_lddt']['max_resolution'])
    )
...
def distogram_loss(out, batch, config):
    ...
    ####### Calculate loss separately for loop######
    mean_renum_loop = mean * mask_renum
    mean_renum_loop = torch.sum(mean_renum_loop, dim=-1)
    mean_renum_loop = torch.mean(mean_renum_loop)
    ################################################
...
def experimentally_resolved_loss(out, batch, config):
    ...
    ####### Calculate loss separately for loop######
    loss_renum_loop = loss * mask_renum
    loss_renum_loop = torch.sum(loss_renum_loop, dim=-1)
    ################################################
    loss = torch.sum(loss, dim=-1)
    if(config['experimentally_resolved']['filter_by_resolution']):
        ...
        loss_renum_loop = loss_renum_loop * (
                (batch['resolution'] >= config['experimentally_resolved']['min_resolution']) & (
                    batch['resolution'] <= config['experimentally_resolved']['max_resolution'])
        )
    ##################
    loss_renum_loop = torch.mean(loss_renum_loop)
    ##################
...
def compute_fape(
    ...
    ########### for loop loss ##########
    if pair_mask is not None:
        renum_pair_mask = renum_mask[..., None]*renum_mask[...,None,:]
        renum_pair_mask = mask * renum_pair_mask
        renum_pair_mask = renum_pair_mask * pair_mask
        normalization_factor_loop = torch.sum(renum_pair_mask, dim=(-1, -2))
        normed_error_loop = torch.sum(normed_error*renum_pair_mask, dim=(-2, -1)) / (eps + normalization_factor_loop)
    else:
        frames_mask_loop = frames_mask.view(-1, 8)
        frames_mask_loop = frames_mask_loop * renum_mask[...,None]
        positions_mask_loop = positions_mask.view(-1, 14)
        positions_mask_loop = positions_mask_loop * renum_mask[..., None]
        renum_pair_mask = torch.flatten(frames_mask_loop)[..., None] * torch.flatten(positions_mask_loop)[..., None, :]
        normalization_factor_loop = torch.sum(renum_pair_mask, dim=(-1, -2))
        normed_error_loop = torch.sum(normed_error*renum_pair_mask, dim=(-2, -1)) / (eps + normalization_factor_loop)
    ...
    return normed_error, normed_error_loop
...
def backbone_loss(
    ...
    fape_loop_loss = torch.mean(fape_loop)
    return fape_loss, fape_loop_loss
...
def supervised_chi_loss(
    ...
    ############ for loop ##########
    chi_mask_loop = chi_mask*renum_mask[...,None]
    sq_chi_loss_loop = masked_mean(chi_mask_loop[None], sq_chi_error, dim=(-1, -2, -3))
    ################################
    ...
    ############ for loop ##########
    angle_norm_loss_loop = masked_mean(renum_mask[None, :, None], norm_error, dim=(-1, -2, -3))
    loss_loop = config['chi_weight']*sq_chi_loss_loop + config['angle_norm_weight']*angle_norm_loss_loop
    ################################
...
def between_residue_bond_loss(
    ...
    #### for loop loss#####
    mask_loop = mask * renum_mask[:-1]
    c_n_loss_loop = torch.sum(mask_loop * c_n_loss_per_residue, dim=-1) / (
            torch.sum(mask_loop, dim=-1) + eps
    )
    #######################
    ...
    #### for loop loss #####
    mask_loop = mask * renum_mask[:-1]
    ca_c_n_loss_loop = torch.sum(mask_loop * ca_c_n_loss_per_residue, dim=-1) / (
            torch.sum(mask_loop, dim=-1) + eps
    )
    ########################
    ...
    #### for loop loss #####
    mask_loop = mask * renum_mask[:-1]
    c_n_ca_loss_loop = torch.sum(mask_loop * ca_c_n_loss_per_residue, dim=-1) / (
            torch.sum(mask_loop, dim=-1) + eps
    )
    ########################
    ...
    return {
        ...
        "c_n_loss_mean_loop": c_n_loss_loop,
        "ca_c_n_loss_mean_loop": ca_c_n_loss_loop,
        "c_n_ca_loss_mean_loop": c_n_ca_loss_loop,
...
def find_structural_violations(
    ...
    return {
        "between_residues": {
            ...
            "bonds_c_n_loss_mean_loop": connection_violations["c_n_loss_mean_loop"],  # ()
            "angles_ca_c_n_loss_mean_loop": connection_violations[
                "ca_c_n_loss_mean_loop"
            ],  # ()
            "angles_c_n_ca_loss_mean_loop": connection_violations[
                "c_n_ca_loss_mean_loop"
            ],  # ()
...
def structural_violation_loss(mask: torch.Tensor,
    ...
    num_atoms_renum = torch.sum(mask*renum_mask[...,None]).type(torch.float32) + 1e-6
    between_residues = violations['between_residues']
    within_residues = violations['within_residues']
    violation_loss = config['structural_violation_loss_weight'] * (between_residues['bonds_c_n_loss_mean'] +
    ...
    violation_loss_loop = config['structural_violation_loss_weight'] *(between_residues['bonds_c_n_loss_mean_loop'] +
           between_residues['angles_ca_c_n_loss_mean_loop']  +
           between_residues['angles_c_n_ca_loss_mean_loop'] +
           torch.sum((between_residues['clashes_per_atom_loss_sum'] +
                   within_residues['per_atom_loss_sum'])*renum_mask[...,None]) / num_atoms_renum
           )
    return violation_loss, violation_loss_loop
...
def tm_loss(
    ...
    loss = loss / denom[..., None]
    ######### loss for renum loop ###########
    loss_renum = loss * mask_renum
    loss_renum = torch.sum(loss_renum, dim=-1)
    loss_renum = loss_renum * (
            (resolution >= min_resolution) & (resolution <= max_resolution)
    )
    loss_renum = torch.mean(loss_renum)
    #########################################
    loss = torch.sum(loss, dim=-1)
    loss = loss * scale
...
def structure_loss(out, batch, config):
    ...
    sup_chi_loss, sup_chi_loss_loop = supervised_chi_loss(seq_mask,
                                        chi_mask,
                                        aatype.long(),
                                        gt_chi_angles,

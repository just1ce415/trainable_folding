...
class TemplateEmbeddingIteration(nn.Module):
    ...
    def forward(self, act, pair_mask):
        act = act.clone()
        act += self.TriangleMultiplicationOutgoing(act.clone(), pair_mask)
        act += self.TriangleMultiplicationIngoing(act.clone(), pair_mask)
        act += self.TriangleAttentionStartingNode(act.clone(), pair_mask)
        act += self.TriangleAttentionEndingNode(act.clone(), pair_mask)
...
class DockerIteration(nn.Module):
    ...
    def forward(self, batch):
        ...
        structure_loss, gt_rigid, gt_affine_mask = loss_multimer.structure_loss(
            out, batch, self.global_config['model']['heads']
        )
        pae_loss, pae_loss_masked = loss_multimer.tm_loss(
            pae_logits[0],
            pae_breaks,
            out['struct_out']['frames'][-1][0],
            gt_rigid,
            gt_affine_mask,
            batch['loss_mask'],
            batch['resolution'][0],
            self.global_config['model']['heads']
        )
        loss = sum([
            0.01 * lddt_loss,
            0.01 * resolved_loss,

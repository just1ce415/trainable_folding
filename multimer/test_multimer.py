...
from multimer import rigid, zero_to_fp32
...
if __name__ == '__main__':
    ...
    # with open('./test/7epe/7epe_A.pdb') as f:
    #    atom_pos = pdb_to_template.from_pdb_string(f.read(), 'A')
    # has_ca = atom_pos.atom_mask[:, 0] == 1
    # print( ''.join([
    #     residue_constants.restypes_with_x[i]
    #     for i in atom_pos.aatype[has_ca]]))
    # quit()
    ...
    #    c_xyz=atom_pos[..., 2, :],
    #)
    processed_feature_dict = pipeline_multimer.process('./test/7epe/7epe.json', config_multimer)
    ...
    ######################### template for new residue ##########################
    has_ca = protein_object_A.atom_mask[:, 0] == 1
    template_aatype = np.expand_dims(np.concatenate((protein_object_A.aatype[has_ca], np.zeros((1,),dtype=int)), axis=0), axis=0)
    template_all_atom_pos = np.expand_dims(np.concatenate((protein_object_A.atom_positions[has_ca],  temp_coor_B), axis=0), axis=0)
    template_all_atom_mask = np.expand_dims(np.concatenate((protein_object_A.atom_mask[has_ca], temp_mask_B), axis=0), axis=0)
    feats['template_aatype'] =  torch.unsqueeze(torch.tensor(template_aatype, device='cuda:0'),0)
    feats['template_all_atom_mask'] =  torch.unsqueeze(torch.tensor(template_all_atom_mask, device='cuda:0'),0)
    feats['template_all_atom_positions'] =  torch.unsqueeze(torch.tensor(template_all_atom_pos, device='cuda:0'),0)
    #############################################################################
    #test_ck = torch.load('/storage/thu/ab_ft_param/checkpoint/zero_pp_rank_0_mp_rank_00_optim_states.pt')
    #ckpoint = zero_to_fp32.get_fp32_state_dict_from_zero_checkpoint('/storage/thu/ab_ft_param/')
    model = modules_multimer.DockerIteration(config_multimer.config_multimer)
    #sd = {k[len("module.model."):]:v for k,v in ckpoint.items()}
    #model.load_state_dict(sd)
    load_param_multimer.import_jax_weights_(model)
    #add_template_feature(feats, './test/7epe/7epe_A.pdb', 'A', 1)

...
class DockerIteration(nn.Module):
    def __init__(self, config, global_config):
        ...
        self.TemplateAngleEmbedder = modules.TemplateAngleEmbedder(config['InputEmbedder'], global_config)
        self.PredictedAlignedError = modules.PredictedAlignedError(config['Predicted_Aligned_Error'], global_config)
    ...
    def modules_to_devices(self):
        ...
        self.TemplateAngleEmbedder.to(self.config['StructureModule']['device'])
        self.PredictedAlignedError.to(self.config['StructureModule']['device'])
    ...
    def forward(self, input, recycling=None):
        ...
        x['msa_mask'] = input["msa"]["main_mask"].to(self.config['Evoformer']['device'])

        if 'template' in input and self.global_config['model']['embed_torsion_angles']:
            temp_act = self.TemplateAngleEmbedder(input)
            x['r1d'] = torch.cat([x['r1d'], temp_act], dim=-3)
            x['msa_mask'] = torch.cat([input["msa"]["main_mask"].to(self.config['Evoformer']['device']), input['template']['template_torsion_angles_mask'][..., 2]], dim=-2)

        for evo_i, evo_iter in enumerate(self.Evoformer):
            if self.config['Evoformer']['EvoformerIteration']['checkpoint']:
                x['r1d'], x['pair'] = checkpoint(evo_iter, x['r1d'], x['pair'], x['msa_mask'])
            else:
        ...
        out_dict['predicted_aligned_error'] = self.PredictedAlignedError(pair)
